...
const color = (text, color) => { return !color ? chalk.green(text) : chalk.keyword(color)(text) };
 ...
 //baner
const banner = cfonts.render("Parchita| Bot|", {
  font: 'pallet',
  align: 'center',
  gradient: ["green","blue"]
})
...
async function startProo() {
  ...
  console.log(chalk.cyanBright("🔥 Parchita"));
...
sock.ev.on('messages.upsert', async m => {
  ...
  try {
    ...
    // ===== FriendlyChat: Borrado automático de groserías =====
    try {
      const isFriendlyActive = isGroup ? friendlyChatList.includes(from) : false;
      // Solo actúa si está activo, es grupo, y quien escribe NO es admin/owner/bot
      if (isFriendlyActive && isGroup && !isOwner && !isBot && !isSenderGroupAdmin) {
        const contieneGroseria = palabrasBaneadas.some(palabra => 
          textToCheck.toLowerCase().includes(palabra.toLowerCase())
        );
        ...
        if (contieneGroseria) {
          ...
          // Opcional: Enviar una advertencia corta
          // await sock.sendMessage(from, { text: `⚠️ @${sender.split('@')[0]}, mantén un lenguaje amigable.`, mentions: [sender] });
        }
      }
    } catch (err) {
      console.log('Error en FriendlyChat filter:', err);
    }
...
case 'apagarbot': {
    ...
    if (args[0] === '1') {
        if (botApagado) return enviar("⚠️ Parchita ya está *Apagada*.");
        botApagado = true;
        enviar("🚫 *Comandos desactivados.* Ahora los gestos y juegos no funcionarán para usuarios.");
    } else if (args[0] === '0') {
        if (!botApagado) return enviar("⚠️ Parchita ya está *Encendida*.");
        botApagado = false;
        enviar("✅ *Comandos activados.* Parchita está lista para la acción.");
    }
    break;
}
...
case 'friendlychat':
  if (!isGroup) return enviar(respuesta.group);
  if (!isSenderGroupAdmin && !isOwner) return enviar(respuesta.admin);
  if (!args[0]) return enviar(`Uso: ${prefix}friendlychat 1 para activar o 0 para desactivar`);

  if (args[0] === '1') {
    if (friendlyChatList.includes(from)) return enviar('El FriendlyChat ya está activado en este grupo.');
    friendlyChatList.push(from);
    saveFriendlyChat();
    enviar('✅ FriendlyChat activado: Ahora limpiaré el lenguaje inapropiado.');
  } else if (args[0] === '0') {
    if (!friendlyChatList.includes(from)) return enviar('El FriendlyChat ya estaba desactivado.');
    friendlyChatList = friendlyChatList.filter(jid => jid !== from);
    saveFriendlyChat();
    enviar('❌ FriendlyChat desactivado.');
  } else {
    enviar('Opción inválida. Usa 1 o 0.');
  }
  break;
